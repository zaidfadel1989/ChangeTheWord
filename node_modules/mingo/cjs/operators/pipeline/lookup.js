...
var import_internal = require("../../aggregator/_internal");
...
const $lookup = (collection, expr, options) => {
  ...
  if (foreignField && localField) {
    const map = import_util.HashMap.init(options.hashFunction);
  ...
  const agg = new import_internal.AggregatorImpl(pipeline, options);
  const opts = { ...options };
  return collection.map((obj) => {
