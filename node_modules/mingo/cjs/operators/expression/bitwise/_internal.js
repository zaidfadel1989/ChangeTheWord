...
function processBitwise(obj, expr, options, compute) {
  (0, import_util.assert)((0, import_util.isArray)(expr), `expression must be an array.`);
  const nums = (0, import_core.computeValue)(obj, expr, null, options);
  if (nums.some(import_util.isNil)) return null;
  (0, import_util.assert)(nums.every(import_util.isNumber), `expression must evalue to array of numbers.`);
  return compute(nums);
}
